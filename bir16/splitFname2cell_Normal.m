...
splitStr4=cellfun(@(x) if length(x)>1; {x{1},[expression_p(2:end) x{2}]},splitStr3,'UniformOutput', false);
splitStr5=[splitStr3{:}];
display(splitStr5);
...
f = (@ (x) feval((@ (list,ind) list{ind}),  {x^2, sqrt(x)},  (mod(x,2)==0)*1 + (mod(x,2)==1)*2));
