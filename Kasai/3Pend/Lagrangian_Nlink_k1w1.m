...
for i=1:N
    ...
    for j=1:N
        ...
        dt_dLddthis{i,j}=dt_dLddthi;
    end
    dt_dLddth=[dt_dLddth;dt_dLddthi];
    tau=[tau;simplify(dt_dLddth(i)-diff(L,thi(i)))];
end
disp('~~~~~~~~~~~~~~');
Lsub=(m(2)*(dthi(1)^2*l(1)^2 + 2*cos(thi(2))*dthi(1)^2*l(1)*lg(2) + dthi(1)^2*lg(2)^2 + 2*cos(thi(2))*dthi(1)*dthi(2)*l(1)*lg(2) + 2*dthi(1)*dthi(2)*lg(2)^2 + dthi(2)^2*lg(2)^2))/2;
dL_ddth

tmp= 2*dthi(1)*l(1)*lg(2)*cos(thi(2))+dthi(2)*l(1)*lg(2)*cos(thi(2));
j=2;
tmp2=diff(tmp,thi(j))*dthi(j)+diff(tmp,dthi(j))*ddthi(j)
latex(tmp2)
